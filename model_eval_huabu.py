...
def visualize_csi_and_mask(clean_csi, full_csi, im_paste,
    ...
    # 设置坐标轴刻度和标签
    x_ticks = np.linspace(0, 128, 5)  # 在 0 到 128 之间生成 5 个刻度
    x_labels = np.linspace(0, 16, 5).astype(int)
    y_ticks = np.linspace(0, 128, 9)  # 在 0 到 128 之间生成 9 个刻度
    ...
    cmap = plt.cm.hot
    plt.imshow(reshaped_csi_masked, cmap=cmap, extent=[0, 128, 0, 128], aspect='auto', vmin=vmin, vmax=vmax)
...
args.output_dir = "/data/liuym/mae-main/output_dir/test_csi_Ws2_2e4_patch2_mask0.9_1103/test_results_test/"
...
checkpoint = torch.load("/data/liuym/mae-main/output_dir/test_csi_Ws2_2e4_patch2_mask0.9_1103/checkpoint-700.pth", map_location="cpu")
...
if args.noise_db:
    ...
    data_dir = f"/home/liuym/csv_data_Ws2_{args.noise_db}_10_test/"
    # data_dir = f"/data/liuym/csv_data_Ws2_{args.noise_db}_FullyCorrelated/"
else:
    # Clean 数据路径（假设格式为 ../csv_data_Ws2_2e4_clean/）
    data_dir = f"/home/liuym/csv_data_Ws2_10_test/"
    # data_dir = f"/data/liuym/csv_data_Ws2_FullyCorrelated/"
csv_files_clean = [f"/home/liuym/csv_data_Ws2_10_test/{i}.csv" for i in range(1, 2000)]
...
# for batch_idx, (noisy_batch, self_infos, _) in enumerate(data_loader_test_noise):
    ...
    with torch.no_grad():
        # 将干净数据作为target传入模型
        loss, NMSE, pred, mask = model(noisy_inputs, self_infos=None, mask_ratio=args.mask_ratio, target=clean_targets)
