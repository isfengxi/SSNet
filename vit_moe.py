...
# vit_moe.py
__all__ = ['PatchEmbed', 'BlockWithMoE', 'VisionTransformer']
...
class MoEBlock(nn.Module):
    def __init__(self, in_features, num_experts=4, top_k=2, hidden_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 选择Top-K专家
        top_k_scores, top_k_indices = torch.topk(gate_scores, self.top_k, dim=-1)  # [B, N, top_k]
        # print("Top-K Indices:", top_k_indices[0, 0])  # 打印第一个样本第一个位置的专家选择

        # 稀疏计算：仅激活选中的专家
        output = torch.zeros_like(x)
        for i in range(self.top_k):
            # 生成 expert_mask，形状为 [B, N, num_experts]
            expert_mask = F.one_hot(top_k_indices[:, :, i], self.num_experts).float()

            # 堆叠专家输出，形状为 [B, N, num_experts, C]
            expert_output = torch.stack([e(x) for e in self.experts], dim=2)  # 注意 dim=2

            # 使用 einsum 进行加权求和
            output += torch.einsum('bnec,bne->bnc', expert_output, expert_mask) * top_k_scores[:, :, i].unsqueeze(-1)
...
class BlockWithMoE(Block):
    def __init__(self, dim, num_heads, num_experts=4, top_k=2, mlp_ratio=4.,
        ...
        print("BlockWithMoE Initialized with params:")  # 调试输出
        print(f"dim={dim}, num_heads={num_heads}, num_experts={num_experts}, top_k={top_k}")
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, use_moe=False, num_experts=4, top_k=2):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            BlockWithMoE(
                ...
                drop_path=dpr[i],
                act_layer=nn.GELU,  # 显式指定或从参数传入
                norm_layer=norm_layer
            ) if use_moe else Block(  # 保留原Block
